...
Повертає керовані диски: ім'я, група, SKU, розмір у ГБ, покоління (~hyperVGeneration~, ~V1~ / ~V2~), тип
безпеки (~securityProfile/securityType~, напр. ~TrustedLaunch~), стан і VM, до якої диск прив'язано.
Виклик — ~GET .../providers/Microsoft.Compute/disks~ (~api-version=2023-04-02~, перевірено через ~az
--debug~). Непривʼязаний диск позначено прямо: він залишається тарифікованим, тож це витік витрат, як і
статична публічна IP без власника (див. [[file:main-tool-pubips.org][~pubips~]]). Опис для ~tools/list~ і диспетчеризація — в
...
виводимо малими літерами, як у [[file:main-tool-vms.org][~vms~]]. Фільтр ~name~ не залежить від регістру. ~hyperVGeneration~ і
~securityProfile/securityType~ важливі для перевикористання диска (напр. створення образу зі снепшота):
непідтримувана комбінація — типова причина, чому ~az disk create --source~ / ~az image create~ падає; у
диска без визначеної безпеки (старіші, не ~TrustedLaunch~) поле відсутнє — тоді ~-~.
