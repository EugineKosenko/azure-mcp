#+title: Інструмент ~dnszones~

Повертає приватні DNS-зони: їхні зв'язки з віртуальними мережами (~virtualNetworkLinks~) і A-записи.  Три
види викликів: перелік зон (~GET .../providers/Microsoft.Network/privateDnsZones~,
~api-version=2018-09-01~), зв'язки зони (~<id зони>/virtualNetworkLinks~, ~api-version=2024-06-01~) і
A-записи (~<id зони>/A~, ~api-version=2018-09-01~). Усі версії перевірено через ~az --debug~. Опис для
~tools/list~ і диспетчеризація — в [[file:main-tools.org][~main-tools.org~]].

Саме зв'язки пояснюють, чи побачить мережа приватну адресу: зона, не зв'язана з мережею клієнта, не
впливає на його резолвінг.

* Модуль

#+begin_src rust :noweb yes :mkdirp yes :tangle src/tools/dnszones.rs
  <<Декларації>>
...
Заголовок: ім'я зони й група. Далі рядок на кожен зв'язок (~зв'язок -> мережа~ з ознакою автореєстрації)
і на кожен A-запис (ім'я, адреси, ~ttl~). Записів без адрес (~aRecords~ порожній) не буває, а от зона без
записів чи зв'язків — нормальна, тоді просто немає відповідних рядків.
...
Зв'язки й записи запитуємо окремими викликами на кожну зону. Помилка будь-якого виклику зупиняє
інструмент і повертається текстом. Фільтр ~zone~ не залежить від регістру імені.
...
Тестова зона: ~privatelink.mysql.database.azure.com~ у ~prod-rg~, зв'язана лише з ~prod-vnet~.
