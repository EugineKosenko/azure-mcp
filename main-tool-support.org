#+title: Інструмент ~support~

Запити підтримки підписки: стан, критичність, служба/класифікація проблеми, дата створення, короткий
заголовок. Виклик — ~GET .../providers/Microsoft.Support/supportTickets~ (~api-version=2020-04-01~),
перевірено живим викликом 2026-09-25. Заміна ~az support in-subscription tickets list~ / ~show~. Опис для
~tools/list~ і диспетчеризація — в [[file:main-tools.org][~main-tools.org~]].

* Модуль

#+begin_src rust :noweb yes :mkdirp yes :tangle src/tools/support.rs
  <<Декларації>>
...
~properties/contactDetails~ (ім'я, телефон, пошта того, хто відкрив запит) відповідь несе, але свідомо не
показуємо: для перевірки стану запиту ці поля не потрібні, а зайвий контакт у тексті інструмента — зайвий
ризик. Елементи не мають групи ресурсів, тож своє сортування — за датою створення, найновіші перші.
...
Зразок — форма реальної відповіді =api= (перевірено живим викликом 2026-09-25), контактні дані замінено
на вигадані й у самому тесті не використовуються (поле інструментом не показується).
