#+title: Інструмент ~vms~

Повертає віртуальні машини: ім'я, група, регіон, розмір, ОС, стан живлення, приватні та публічні IP,
безпека завантаження (~securityType~, ~secureBoot~, ~vTpm~). Офіційний
=Azure MCP= цього нормально не дає, а ~az vm list -d~ робить окремий виклик на кожну VM (для 17 машин —
понад 50 запитів). Тут вистачає чотирьох переліків по області:

- VM (~GET .../providers/Microsoft.Compute/virtualMachines~, ~api-version=2024-11-01~): розмір, ОС, NIC;
- ті самі VM зі ~statusOnly=true~ (та сама версія): у відповіді лише ~instanceView.statuses~, з них
  беремо ~PowerState/…~;
- NIC (~api-version=2023-11-01~) і публічні IP (~api-version=2024-07-01~) — для адрес.

Версії перевірено через ~az --debug~, а форму відповіді ~statusOnly~ — живим читанням. Опис для
~tools/list~ і диспетчеризація — в [[file:main-tools.org][~main-tools.org~]].

* Модуль

#+begin_src rust :noweb yes :mkdirp yes :tangle src/tools/vms.rs
  <<Декларації>>

  <<Визначення>>

  pub async fn run(http: &reqwest::Client, arguments: &serde_json::Value) -> serde_json::Value {
      <<Дії>>
  }

  #[cfg(test)]
  mod tests {
      use super::*;

      <<Тести>>
  }
#+end_src

* Індекс за ідентифікатором

Переліки з'єднуємо за ідентифікатором ресурсу без огляду на регістр: у переліку VM група пишеться
~DEMO-RG~, а в ідентифікаторі NIC тієї самої VM — ~demo-rg~.

#+begin_src rust :noweb-ref Декларації
  use std::collections::HashMap;

  use crate::client;
  use crate::tools;
#+end_src

#+begin_src rust :noweb-ref Визначення
  fn index(items: &[serde_json::Value]) -> HashMap<String, &serde_json::Value> {
      items.iter().map(|item| (tools::text(item, "/id").to_lowercase(), item)).collect()
  }
#+end_src

* Стан живлення

~instanceView.statuses~ — перелік ~{code, displayStatus}~: один запис про виділення
(~ProvisioningState/…~), інший про живлення (~PowerState/running~, ~PowerState/deallocated~ тощо). Беремо
суфікс після ~PowerState/~. Якщо запису про VM немає (VM зникла між двома викликами) чи стану в ній нема
— ~-~: це нормальна відсутність, а не помилка.

#+begin_src rust :noweb-ref Визначення
  fn power(status: Option<&&serde_json::Value>) -> String {
      status
          .and_then(|item| item["properties"]["instanceView"]["statuses"].as_array())
          .into_iter()
          .flatten()
          .filter_map(|item| item["code"].as_str()?.strip_prefix("PowerState/"))
          .next()
          .unwrap_or("-")
          .to_string()
  }
#+end_src

* Адреси

Приватні адреси беремо з IP-конфігурацій усіх NIC машини, публічні — з тих конфігурацій, до яких
прив'язано публічну IP. Кілька адрес виводимо через кому; відсутні — ~-~.

#+begin_src rust :noweb-ref Визначення
  fn joined(items: Vec<&str>) -> String {
      if items.is_empty() { "-".to_string() } else { items.join(",") }
  }

  fn addresses<'a>(
      vm: &serde_json::Value,
      nics: &HashMap<String, &'a serde_json::Value>,
      ips: &HashMap<String, &'a serde_json::Value>,
  ) -> (String, String) {
      let (mut private, mut public) = (Vec::new(), Vec::new());

      for link in vm["properties"]["networkProfile"]["networkInterfaces"].as_array().into_iter().flatten() {
          let Some(nic) = nics.get(&tools::text(link, "/id").to_lowercase()) else { continue };

          for config in nic["properties"]["ipConfigurations"].as_array().into_iter().flatten() {
              private.push(tools::text(config, "/properties/privateIPAddress"));

              if let Some(ip) = ips.get(&tools::text(config, "/properties/publicIPAddress/id").to_lowercase()) {
                  public.push(tools::text(ip, "/properties/ipAddress"));
              }
          }
      }

      (joined(private), joined(public))
  }
#+end_src

* Таблиця

Рядок: ім'я, група, регіон, розмір, ОС, стан, приватні й публічні адреси, безпека завантаження, дата
створення (~timeCreated~, у кінці рядка — нове поле в наявному інструменті йде туди, а не всередину, щоб
не ламати позиційний розбір споживачів). Групу
виводимо малими літерами, як у ~costs~: імена груп в =Azure= нечутливі до регістру, а перелік VM повертає
їх у верхньому. Фільтр ~name~ не залежить від регістру. Регіон беремо окремо від групи: назва групи
ресурсів (напр. ~prod-eastus~) підказує регіон лише за конвенцією проєкту, а не гарантовано — VM може
фізично лежати в іншому. ~securityType~ (~TrustedLaunch~ тощо) є не в кожної VM (старі чи ~Standard~ без
довіреного завантаження); тоді й ~secureBootEnabled~ / ~vTpmEnabled~ відсутні — все разом дає ~-~.

#+begin_src rust :noweb-ref Визначення
  fn security(vm: &serde_json::Value) -> String {
      let kind = tools::text(vm, "/properties/securityProfile/securityType");

      if kind == "-" {
          return "-".to_string();
      }

      let mut flags = Vec::new();

      if vm["properties"]["securityProfile"]["uefiSettings"]["secureBootEnabled"].as_bool() == Some(true) {
          flags.push("secureBoot");
      }
      if vm["properties"]["securityProfile"]["uefiSettings"]["vTpmEnabled"].as_bool() == Some(true) {
          flags.push("vTpm");
      }

      format!("{}({})", kind, flags.join(","))
  }

  fn table(vms: &[serde_json::Value], status: &[serde_json::Value], nics: &[serde_json::Value], ips: &[serde_json::Value], name: Option<&str>) -> String {
      let (status, nics, ips) = (index(status), index(nics), index(ips));

      let lines: Vec<String> = tools::sorted(vms)
          .into_iter()
          .filter(|vm| name.is_none_or(|name| tools::same(tools::text(vm, "/name"), name)))
          .map(|vm| {
              let (private, public) = addresses(vm, &nics, &ips);

              format!(
                  "{}  {}  {}  {}  {}  {}  private={}  public={}  security={}  created={}",
                  tools::text(vm, "/name"),
                  tools::part(tools::text(vm, "/id"), "resourceGroups").to_lowercase(),
                  tools::text(vm, "/location"),
                  tools::text(vm, "/properties/hardwareProfile/vmSize"),
                  tools::text(vm, "/properties/storageProfile/osDisk/osType"),
                  power(status.get(&tools::text(vm, "/id").to_lowercase())),
                  private,
                  public,
                  security(vm),
                  tools::text(vm, "/properties/timeCreated"),
              )
          })
          .collect();

      if lines.is_empty() { "Віртуальних машин не знайдено.".to_string() } else { lines.join("\n") }
  }
#+end_src

* Дія

VM беремо в межах ~group~ (якщо задано), а NIC і публічні IP — по всій підписці: NIC машини може лежати в
іншій групі. Помилка будь-якого з чотирьох викликів зупиняє інструмент і повертається текстом.

#+begin_src rust :noweb-ref Визначення
  async fn gather(http: &reqwest::Client, arguments: &serde_json::Value) -> Result<String, String> {
      let scope = tools::scope(arguments).await?;
      let subscription = tools::subscription(arguments).await?;

      let vms = client::list(http, &format!("{}/providers/Microsoft.Compute/virtualMachines?api-version=2024-11-01", scope)).await?;
      let status = client::list(http, &format!("{}/providers/Microsoft.Compute/virtualMachines?statusOnly=true&api-version=2024-11-01", scope)).await?;
      let nics = client::list(http, &format!("{}/providers/Microsoft.Network/networkInterfaces?api-version=2023-11-01", subscription)).await?;
      let ips = client::list(http, &format!("{}/providers/Microsoft.Network/publicIPAddresses?api-version=2024-07-01", subscription)).await?;

      Ok(table(&vms, &status, &nics, &ips, arguments["name"].as_str()))
  }
#+end_src

#+begin_src rust :noweb-ref Дії
  tools::reply(gather(http, arguments).await)
#+end_src

* Перевірки

VM ~api-vm~: група в її ідентифікаторі велика, у NIC мала; NIC має публічну IP. Друга VM зупинена, без
NIC у переліку.

#+begin_src rust :noweb-ref Тести
  fn vms() -> Vec<serde_json::Value> {
      vec![
          serde_json::json!({
              "name": "api-vm",
              "id": "/subscriptions/s/resourceGroups/DEMO-RG/providers/Microsoft.Compute/virtualMachines/api-vm",
              "location": "eastus",
              "properties": {
                  "hardwareProfile": { "vmSize": "Standard_B1ms" },
                  "storageProfile": { "osDisk": { "osType": "Linux" } },
                  "networkProfile": { "networkInterfaces": [{ "id": "/subscriptions/s/resourceGroups/demo-rg/providers/Microsoft.Network/networkInterfaces/nic1" }] },
                  "securityProfile": { "securityType": "TrustedLaunch", "uefiSettings": { "secureBootEnabled": true, "vTpmEnabled": true } },
                  "timeCreated": "2026-09-20T07:06:17.5125651+00:00",
              },
          }),
          serde_json::json!({
              "name": "win-server",
              "id": "/subscriptions/s/resourceGroups/TRIAL/providers/Microsoft.Compute/virtualMachines/win-server",
              "location": "westus3",
              "properties": {
                  "hardwareProfile": { "vmSize": "Standard_D2s_v3" },
                  "storageProfile": { "osDisk": { "osType": "Windows" } },
                  "networkProfile": { "networkInterfaces": [{ "id": "/subscriptions/s/resourceGroups/trial/providers/Microsoft.Network/networkInterfaces/missing" }] },
              },
          }),
      ]
  }

  fn status() -> Vec<serde_json::Value> {
      vec![
          serde_json::json!({
              "id": "/subscriptions/s/resourceGroups/DEMO-RG/providers/Microsoft.Compute/virtualMachines/api-vm",
              "properties": { "instanceView": { "statuses": [{ "code": "ProvisioningState/succeeded" }, { "code": "PowerState/running" }] } },
          }),
          serde_json::json!({
              "id": "/subscriptions/s/resourceGroups/TRIAL/providers/Microsoft.Compute/virtualMachines/win-server",
              "properties": { "instanceView": { "statuses": [{ "code": "PowerState/deallocated" }] } },
          }),
      ]
  }

  fn nics() -> Vec<serde_json::Value> {
      vec![serde_json::json!({
          "id": "/subscriptions/s/resourceGroups/demo-rg/providers/Microsoft.Network/networkInterfaces/nic1",
          "properties": { "ipConfigurations": [{
              "properties": {
                  "privateIPAddress": "10.0.0.5",
                  "publicIPAddress": { "id": "/subscriptions/s/resourcegroups/demo-rg/providers/microsoft.network/publicipaddresses/ip1" },
              },
          }] },
      })]
  }

  fn ips() -> Vec<serde_json::Value> {
      vec![serde_json::json!({
          "id": "/subscriptions/s/resourceGroups/demo-rg/providers/Microsoft.Network/publicIPAddresses/ip1",
          "properties": { "ipAddress": "203.0.113.40" },
      })]
  }

  #[test]
  fn table_joins_status_and_addresses() {
      assert_eq!(
          table(&vms(), &status(), &nics(), &ips(), None),
          "api-vm  demo-rg  eastus  Standard_B1ms  Linux  running  private=10.0.0.5  public=203.0.113.40  security=TrustedLaunch(secureBoot,vTpm)  created=2026-09-20T07:06:17.5125651+00:00\n\
           win-server  trial  westus3  Standard_D2s_v3  Windows  deallocated  private=-  public=-  security=-  created=-"
      );
  }

  #[test]
  fn table_filters_by_name() {
      assert!(table(&vms(), &status(), &nics(), &ips(), Some("API-VM")).starts_with("api-vm"));
      assert_eq!(table(&vms(), &status(), &nics(), &ips(), Some("other")), "Віртуальних машин не знайдено.");
  }

  #[test]
  fn power_missing_is_dash() {
      assert_eq!(power(None), "-");
      assert_eq!(
          table(&vms(), &[], &nics(), &ips(), Some("win-server")),
          "win-server  trial  westus3  Standard_D2s_v3  Windows  -  private=-  public=-  security=-  created=-"
      );
  }

  #[test]
  fn security_without_uefi_settings_has_no_flags() {
      let vm = serde_json::json!({ "properties": { "securityProfile": { "securityType": "TrustedLaunch" } } });

      assert_eq!(security(&vm), "TrustedLaunch()");
  }
#+end_src
