#+title: Інструмент ~costs~

Повертає фактичні витрати (~ActualCost~) за період: по підписці чи по одній групі ресурсів, з розбивкою
за ~ResourceGroupName~, ~ResourceId~, ~ResourceType~, ~MeterCategory~ або ~ServiceName~. Виклик — ~POST
.../providers/Microsoft.CostManagement/query~ (~api-version=2023-11-01~), тіло описує вибірку. Опис для
~tools/list~ і диспетчеризація — в [[file:main-tools.org][~main-tools.org~]].

Відповідь має ~properties.columns~ (масив ~{name, type}~) і ~properties.rows~ (масив масивів у порядку
колонок; колонка ~Currency~ додається сама, а за щоденної деталізації ще й ~UsageDate~). Порядок колонок
не вгадуємо, а знаходимо за назвою.

Два режими виводу. Звичайний — одна таблиця «сума за період» за вибраною розбивкою. Щоденний (~daily~) —
витрати за кожну добу й середня за повну добу для кожної групи; це те, з чого користувач сам рахує ставку
й прогноз.

* Модуль

#+begin_src rust :noweb yes :mkdirp yes :tangle src/tools/costs.rs
  <<Декларації>>
...
Перелічені значення збігаються з ~enum~ у схемі інструмента, а сервер перевіряє їх сам: помилка в назві
розбивки дала б від =Azure= малозрозумілу відмову.
...
~from~ і ~to~ задають діапазон дат (включно) у форматі ~РРРР-ММ-ДД~ і перекривають ~period~. Дати
перевіряємо самі: за хибного формату =Azure= відповідає загальною відмовою.
...
Для довільного періоду ~timeframe~ дорівнює ~Custom~, а межі йдуть у ~timePeriod~ (від початку першої
доби до кінця останньої, ~UTC~). ~granularity~ — ~Daily~ для щоденного режиму, інакше ~None~.
...
~ResourceId~ — довгий шлях у нижньому регістрі; читачеві потрібні група, тип і ім'я. Із шляху
~/subscriptions/…/resourcegroups/trial/providers/microsoft.compute/disks/win-server_osdisk_1~ беремо
групу (~trial~), тип (~disks~) і останній сегмент як ім'я. Вкладений ресурс має пари «тип / ім'я» одна за
одною (~virtualmachines/vm/extensions/ext~), тож типи з'єднуємо скісною рискою. Ідентифікатор без
~providers~ (вироджений випадок) лишаємо як є.
...
Колонки знаходимо за назвою: ~Cost~, розбивка й ~Currency~. Рядки впорядковуємо від більших витрат до
менших і додаємо підсумок; валюту беремо з першого рядка (у підписці вона одна). Якщо відповідь має
~nextLink~, показано не все — про це повідомляємо, а не мовчимо.
...
За ~granularity=Daily~ рядок відповіді — це доба й група. ~UsageDate~ приходить числом ~20260901~, а може
й текстом; приводимо до ~РРРР-ММ-ДД~. Виводимо суму за кожну добу, а нижче — по групах: суму за період і
середню за *повну* добу. Остання доба в даних завжди неповна: =Cost Management= відстає приблизно на
півдоби, і без позначки її легко прочитати як падіння витрат. Тому її не беремо в середнє й прямо
позначаємо. Якщо в даних лише одна доба, середнього немає, а виводимо ~-~.
...
* Примітки до відповіді

У звичайному режимі дат немає, тож додаємо загальну примітку про відставання =Cost Management=. Якщо
відповідь прийшла з кешу, кажемо, скільки їй хвилин: інакше можна сприйняти давні дані за поточні.
...
Область (підписка чи група ресурсів) збирає ~tools::scope~, див. [[file:main-tools.org][~main-tools.org~]]. ~from~ і ~to~ мають
бути разом. Кількість викликів до =Cost Management= варто оцінювати перед серією: кожен промах кешу
коштує ліміту, спільного для тенанта.
...
Розбір відповіді не потребує мережі, тож перевіряється звичайними тестами. Колонки навмисно йдуть не в
тому порядку, що в живій відповіді, — щоб перевірити пошук за назвою.
...
Тіло запиту: звичайний і довільний період, щоденний режим, перевірка дат.
...
Скорочення ресурсу: приклад із диском користувача, вкладений ресурс і вироджений шлях.
...
Щоденний режим: три доби, остання неповна лише в групи ~a~, тож у групи ~b~ немає рядка за неї. Середнє
за повну добу рахується за двома повними добами (перша й друга).
