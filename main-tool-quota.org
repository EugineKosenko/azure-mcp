#+title: Інструмент ~quota~

Квоти регіону: ~compute~ (родини VM, ~Microsoft.Compute/locations/<регіон>/usages~,
~api-version=2024-11-01~, заміна ~az vm list-usage~) і ~network~ (публічні IP, VNet, NSG тощо,
~Microsoft.Network/locations/<регіон>/usages~, ~api-version=2024-05-01~, заміна ~az network
list-usages~) — обидва перевірено живими викликами 2026-09-25. Елементи цих переліків не мають форми
звичайного ресурсу (~name~ — об'єкт ~{value, localizedValue}~, не рядок; ~compute~ і без ~id~), тож
~tools::sorted~ тут не застосовний — своє сортування. Опис для ~tools/list~ і диспетчеризація — в
[[file:main-tools.org][~main-tools.org~]].

* Модуль

#+begin_src rust :noweb yes :mkdirp yes :tangle src/tools/quota.rs
  <<Декларації>>
...
Кожен рядок починається міткою ~compute~ / ~network~ — сортування рядків як текстів природно групує їх і
впорядковує за назвою квоти всередині групи, без потреби в окремому ключі. Вичерпану квоту (використано
не менше ліміту, ліміт додатний) позначено прямо.
...
Зразок — скорочена реальна відповідь обох =api= (2026-09-25).
