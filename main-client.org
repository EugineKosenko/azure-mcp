...
На відміну від ~list~, деякі адреси (напр. сама підписка) віддають один об'єкт, без обгортки ~value~.
...
* Роздрібні ціни ~Azure Retail Prices~

Публічний =api= без автентифікації (~prices.azure.com~): ціни зберігання за регіон. На відміну від решти
клієнта, тут не потрібен ані токен ~az~, ані заголовок =Authorization=, ані підписка — інший хост, не
=arm=. Структура відповіді теж інша: елементи в полі ~Items~ (не ~value~), сторінкування через
~NextPageLink~ (не ~nextLink~), тож не можна перевикористати ~list~.
